...
export function MessagePage() {
  ...
  const isMicrosoftGraph = currentAccount?.provider === 'MicrosoftGraph'
  const { data: commercialSubscription } = useQuery({
    queryKey: ['commercial-subscription'],
    queryFn: commercialApi.subscription,
    staleTime: 30_000,
  })
  const hasMailActions = commercialSubscription?.entitlements.includes(commercialEntitlements.mailActions) === true && !isMicrosoftGraph
  const hasTracking = commercialSubscription?.entitlements.includes(commercialEntitlements.trackingBasic) === true && !isMicrosoftGraph
  const canUnsubscribe = Boolean(messageId && !isMicrosoftGraph)
  ...
  return <article className={`mail-view message-reader ${preview ? 'with-preview' : ''}`}>
    <section className="message-reading-pane">
      ...
      {isMicrosoftGraph && <div className="notice">Microsoft 365 está en Phase 1. Puedes leer el correo y actualizar su estado de lectura; responder, reenviar, mover, eliminar, descargar adjuntos y usar seguimiento aún no está disponible.</div>}
      {(hasMailActions || hasTracking || (canUnsubscribe && message.unsubscribeUrl)) && <div className="message-actions message-action-toolbar unified-message-actions" aria-label="Acciones del correo">
        ...
        {canUnsubscribe && message.unsubscribeUrl && <a className="message-action-button" href={message.unsubscribeUrl} target="_blank" rel="noopener noreferrer" aria-label="Desuscribirse" title="Desuscribirse"><Ban size={16} /><span>Desuscribirse</span></a>}
      ...
      {!isMicrosoftGraph && message.attachments.length > 0 && <div className="attachments"><h2><Paperclip size={16} /> Adjuntos</h2><div className="attachment-list">{message.attachments.map(file => <div key={file.id} className={`attachment-card ${preview?.id === file.id ? 'selected' : ''}`}><button type="button" className="attachment-preview-button" onClick={() => setPreview(file)} title="Abrir vista previa"><Paperclip size={17} /><span><strong>{file.name}</strong><small>{Math.max(1, Math.round(file.size / 1000))} KB · Vista previa</small></span></button><a href={mailApi.attachmentUrl(accountId, messageId, file, true)} className="attachment-download" title={`Descargar ${file.name}`} aria-label={`Descargar ${file.name}`}><Download size={16} /></a></div>)}</div></div>}

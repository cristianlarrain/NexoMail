...
function providerLabel(account: MailAccount) {
  if (account.provider === 'Gmail') return 'Gmail'
  if (account.provider === 'MicrosoftGraph') return 'Microsoft 365'
...
export function AccountsPage() {
  ...
  return <section className="settings-page">
    ...
    {params.get('connected') === 'google' && <div className="success-notice">La cuenta Gmail fue conectada correctamente.</div>}
    ...
    <div className="settings-card">
      <div className="settings-card-header">
        ...
        <div className="settings-provider-actions">
          <button className="secondary-button" disabled={accountLimitReached} title={accountLimitReached ? 'Ha alcanzado el límite de cuentas de su plan.' : 'Agregar una cuenta Microsoft 365'} onClick={() => window.location.assign('/api/oauth/microsoft/start')}><MailPlus size={16} /> Agregar Microsoft 365</button>
          <button className="primary-button" disabled={accountLimitReached} title={accountLimitReached ? 'Ha alcanzado el límite de cuentas de su plan.' : 'Agregar una cuenta Gmail'} onClick={() => window.location.assign('/api/oauth/google/start')}><MailPlus size={16} /> Agregar Gmail</button>
        </div>
      </div>
      {accounts.map(account => <div className="account-row" key={account.id}><i className="account-dot large" style={{ background: account.color }} /><div><strong>{account.displayName}</strong><span>{account.emailAddress}</span></div><span className="provider-label">{providerLabel(account)}</span><span className="connected"><CheckCircle2 size={16} /> Conectada</span><button className="icon-button" aria-label={`Editar ${account.displayName}`} title="Editar cuenta" onClick={() => openEdit(account)}><Pencil size={17} /></button><button className="icon-button danger-icon" aria-label={`Quitar ${account.displayName}`} title="Quitar cuenta" disabled={remove.isPending} onClick={() => setRemoveCandidate(account)}><Trash2 size={17} /></button></div>)}
    </div>

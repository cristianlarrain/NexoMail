...
export function ComposePage() {
  ...
  const send = useMutation({
    mutationFn: () => {
      const payload = { fromAccountId, to: to.split(',').map(v => v.trim()).filter(Boolean), cc: cc.split(',').map(v => v.trim()).filter(Boolean), bcc: [], subject, htmlBody: body || '<p></p>', attachments }
  ...
  return <section className="compose-page"><div className={`compose-card ${!showComposer ? 'ai-first-compose' : ''}`}>
    <header><div><p className="eyebrow">{!origin && !showComposer ? 'Nexo IA' : 'Correo'}</p><h1>{!origin && !showComposer ? 'Asistente de redacción' : action}</h1></div><button className="icon-button" onClick={() => navigate(-1)} aria-label="Cerrar"><X size={19} /></button></header>
    ...
    <form onSubmit={submit}>
      ...
      {showComposer && <>
        ...
        {showCc ? <div className="compose-field"><label>CC</label><input value={cc} onChange={e => setCc(e.target.value)} placeholder="copia@dominio.cl" /></div> : <button type="button" className="text-button" onClick={() => setShowCc(true)}>Agregar CC / CCO</button>}

...
export function AdminPlansPage() {
  ...
  return <section className="settings-page commercial-admin-page">
    ...
    <div className="commercial-admin-guidance"><ShieldCheck size={18} /><span>Freemium se conserva como plan base. Un tipo de cuenta con usuarios asignados no puede eliminarse; primero debe reasignarlos o dejar el plan inactivo.</span></div>
    ...
    <div className="commercial-admin-table-wrap">
      <table className="commercial-admin-table">
        <thead><tr><th>Tipo</th><th>Precio</th><th>Límite</th><th>Usuarios</th><th>Estado</th><th aria-label="Acciones" /></tr></thead>
        <tbody>
    ...
    {editing && <div className="modal-backdrop" role="presentation"><form className="account-modal commercial-plan-editor" onSubmit={event => { event.preventDefault(); save.mutate() }}>
      ...
      <label>Características <small>Una por línea</small><textarea rows={6} value={draft.features} onChange={event => setDraft(current => ({ ...current, features: event.target.value }))} required /></label>
      ...
      <footer><button type="button" className="secondary-button" onClick={() => setEditing(null)}>Cancelar</button><button className="primary-button" disabled={save.isPending}>{save.isPending ? 'Guardando…' : 'Guardar cambios'}</button></footer>
    </form></div>}

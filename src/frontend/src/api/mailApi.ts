...
export const mailApi = {
  ...
  priorityOverrides: (accountId?: string) => api<Array<{ accountId: string; messageId: string }>>(`/mail/control-center/priority-overrides${accountId ? `?accountId=${encodeURIComponent(accountId)}` : ''}`),
  setPriorityOverride: (accountId: string, messageId: string, suppressed: boolean) => api<void>(`/mail/control-center/priority-overrides/${encodeURIComponent(accountId)}/${encodeURIComponent(messageId)}`, { method: 'POST', body: JSON.stringify({ suppressed }) }),
  ...
  updateControlCenterState: (accountId: string, conversationId: string, payload: { messageId: string; action: 'resolved' | 'snoozed' | 'active'; snoozeHours?: number }) => api<void>(`/mail/control-center/${encodeURIComponent(accountId)}/${encodeURIComponent(conversationId)}/state`, { method: 'POST', body: JSON.stringify(payload) }),
